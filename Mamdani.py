...
class FuzzyMethods:
    ...
    # Min-активизация + аккумуляция с помощью max-объединения
    def accum_activation(self, x_value, var):  # HERE: make more activation modes
        self.accum_act = []
        for conclusion in self.aggr:
            self.accum_act.append(min(self.aggr[conclusion], var.mf(x_value)[list(self.aggr.keys()).index(conclusion)]))
